...
- **The OS lock is the layer that holds.** The scheduled scripts run
  ...
  chmod'd unreadable, so the run cannot open it no matter what it decides
  ...
  why the lock, not the hook, carries the boundary. Windows has no chmod
  that denies reads, so Windows brains have only the hook layer today.
