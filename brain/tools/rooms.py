...
def convos_for(srcname, ended=False, limit=6):
    ...
    for c in (data.get("convos") or []):
        if bool(c.get("ended")) != ended or c.get("src") != srcname:
            continue
        out.append({
            "id": c.get("id") or "",
...
function verCheck(){
  fetch('/api/version').then(function(r){ return r.json(); }).then(function(j){
    ...
    if(j.version !== VER){
      ...
      var ask = document.getElementById('askpanel');
      if(ask && !ask.hidden) return;   // never reload under an open chat
      location.reload();
    }
  }).catch(function(){});
}
