...
def week_strip(cfg, today, today_md=""):
    ...
    return (f'<details class="weekstrip"{openattr}><summary>This week'
            ...
            + f'</summary>{sketch}<div class="wcols">{"".join(cols)}</div></details>')
...
def build():
    ...
    if today_md.strip() or habits:
        ...
        if today_md.strip():
            ...
            if ev_done or ev_open:
                head = ("All of it landed &mdash; clean close." if not ev_open
                        else f"{ev_done} of {ev_done + ev_open} done. The open "
                             "ones need a decision &mdash; carry into tomorrow, "
                             "or drop out loud.")
    ...
    if b["live"]:
        ...
        V["todayrail"].append(
            '<section class="railcard"><h3 class="area">Where your attention '
            "went</h3>"
            + "".join(frows)
            + '<p class="hnote">Each front, by when anything in it last moved '
            "&mdash; longest quiet on top.</p></section>")
...
.qinline{display:flex;gap:6px;align-items:center}
...
.pwhere{font:600 var(--t-sm)/1.3 var(--sans);color:var(--dim)}
.pwhere select{font:inherit;color:var(--ink);background:var(--surface);
