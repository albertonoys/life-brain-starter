...
# text can link "Marlow" straight to Marlow on the People tab.
PERSON_NAMES = []
...
def prepared_fold(wsname, open_fresh=False):
    ...
    thing they belong to — the train options live on the Marlow hero, not
...
def room_labels(cfg):
    ...
    "MTR — Champagne dossier for Ari" is the workstream's full name and
...
def stackrow(w, rank, cfg):
    ...
    &middot; Ari" still earns its place: that one tells you not to bother.
    """
    pct = round(decay(w, cfg) * 100)
...
def build():
    ...
    # Task text can now point at people: longest names first so "Blair Chang"
    # wins over "Blair". Skip very short names — too many false hits.
    ...
    # so the train options surface on the Marlow hero, not only in the
    ...
    if today_md.strip() or habits:
        ...
        if today_md.strip():
            # People and workstreams named in the plan are doors: Arden opens
    ...
    # Marlow, whose trip the hero at the top of the page is about.
    #
    ...
    if people:
        ...
        # under five letters only accent/case variants (Ines/Shay) qualify.
  ...
  <div id="runhistory" class="runs"></div>
    ...
    if recs or haves:
        ...
        V["clauderail"].append(
            ...
            '(Jules, moquette, évacuation…)" autocomplete="off"></div>'
            + "".join(rows)
...
""" + _dumpcopy(SHEET, fresh=not b["live"] and not people, cfg=cfg) + SCRIPT + PEOPLE_SCRIPT
    ...
    try:
        ...
        new = re.sub(r'<div class="askscrim".*</script>',
...
.hero-next .box.tick{flex:none;align-self:center}
...
/* How booked a day already is, at a glance: the fuller the day, the deeper
   the wash. Free days keep the plain background — that is the signal. */
.szday.szbz1{background:color-mix(in oklch,var(--dim) 9%,var(--surface))}
.szday.szbz2{background:color-mix(in oklch,var(--dim) 20%,var(--surface))}
...
(function(){
  ...
  document.querySelectorAll('[data-wsaddp]').forEach(function(b){
    b.onclick = function(ev){
      ...
      askDlg({title: 'Link a person \\u2014 ' + b.dataset.wsaddp,
              ...
              f1: {label: 'Who (exact name)', placeholder: 'Dad, Teagan, Maman\\u2026'},
              go: 'Link them'},
        function(o){
          var v = (o.v1 || '').trim();
          if(!v) return;
  ...
  function busyNow(){
    ...
    return sorter || typing || vis('askpanel') || vis('dumpover')
...
def repetition_report(path=None):
    ...
    # narration — and two paragraphs about the same afternoon share "Ari"
