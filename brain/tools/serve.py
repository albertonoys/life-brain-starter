...
# Dictation audio rides in as base64 — a few minutes of speech is well past
# the ordinary cap, and cutting the connection mid-upload showed on the page
# as "could not reach the brain", which reads as a dead server, not a limit.
MAX_BODY_AUDIO = 36 * 1024 * 1024
...
def rebuild(map_too=True):
    ...
    try:
        for job in (["build.py"]
                    + (["map.py", "rooms.py", "proto.py", "routines.py"]
                       if map_too else [])):
            try:
...
def task_action(src, key, action, until="", text=""):
    ...
    if src == "season.md" and action == "done":
        ...
        for nm in (m_w.group(1).split(",") if m_w else []):
            ...
            try:
                ...
                # "Casey" should still find "## Casey Frankie" — but only
    ...
    if src == "workstreams.md" and action in ("done", "defer", "drop"):
        ...
        if owner:
            ...
            # Ari email once TapGate stepped aside.
    ...
    if src == "today.md" and action in ("done", "undone"):
        try:
            ...
            # verbatim, and a run that paraphrases ("Send email to Ari
            # confirming Tuesday" for "Send email to Ari (De Oakley)
...
def plan_edit(op, key="", pick="", day=""):
    ...
    if op in ("kick", "swap"):
        ...
        gone = short(_bare(m.group(3)))
        del lines[i]
        _plan_trail(lines, f"moved “{gone}” to {d.strftime('%A')} {d.day} {d.strftime('%B')}")
        said = f"Moved to {d.strftime('%A')} ✓"
...
@serialized
def next_done(name):
    """Her "did it" on a hero whose Do-this line is the Next: field rather
    than a checkbox. The field line goes, the touch clock resets, and the
    next open task inherits the slot on rebuild."""
    lines = read(WORKSTREAMS).split("\n")
    start = None
    for i, line in enumerate(lines):
        h = re.match(r"^##\s+(.*)$", line.strip())
    ...
    for j in range(start + 1, len(lines)):
        if re.match(r"^##\s+", lines[j].strip()):
            ...
            break
    for j in range(start + 1, end):
        if re.match(r"^\s*-\s+\*\*Next:\*\*", lines[j], re.I):
            del lines[j]
            write(WORKSTREAMS, "\n".join(lines))
            set_field(name, "Touched", date.today().isoformat())
            return
    raise ValueError("no next move is recorded on it")
...
def save_files(files):
    ...
    for f in files[:MAX_ATTACHMENTS]:
        ...
        if ext == ".docx":
            # Claude's Read tool has no Word parser, so every session that
            # got a .docx re-derived one from unzip and XML, in front of her.
            # Convert once at upload (textutil ships with macOS) and hand the
            # session the text twin; the original stays next to it for her.
            twin = os.path.splitext(target)[0] + ".txt"
            try:
                subprocess.run(["textutil", "-convert", "txt", target,
                                "-output", twin],
                               check=True, timeout=20, capture_output=True)
                saved[-1] = os.path.relpath(twin, BRAIN)
            except Exception:
                pass          # the .docx path still works, just clumsier
    return saved
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _body(self):
        ...
        cap = MAX_BODY_AUDIO if self.path == "/api/dictate" else MAX_BODY
        if n > cap:
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/api/sessions/room"):
            ...
            q = parse_qs(urlparse(self.path).query)
            src = (q.get("src") or [""])[0]
            out = {"convos": ROOMS.convos_for(src)}
            # hist=1 adds the ended conversations too — the Ask panel's
            # "your chats" list. The rooms page never asks for these.
            if (q.get("hist") or [""])[0]:
                out["history"] = ROOMS.convos_for(src, ended=True, limit=30)
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path == "/api/touch":
                ...
                rebuild()
                return self._json({"ok": True})
            if self.path == "/api/nextdone":
                next_done(body.get("name", ""))
                rebuild()
                return self._json({"ok": True})
            ...
            if self.path == "/api/me/photo":
                ...
                # (No local `import base64` here: a function-scope import
                # makes the name local to ALL of do_POST, and every handler
                # above this line then dies with UnboundLocalError.)
            ...
            if self.path == "/api/dictate":
                # Push-to-talk dictation for browsers without a speech API
                # (Zen/Firefox): the page records, this Mac transcribes with
                # the same local Whisper the voice notes use. Nothing leaves
                # the machine, and the audio is gone when the request ends.
                ...
                if len(blob) > 25_000_000:
                    raise ValueError("that recording is very long — send it "
                                     "as a voice note instead")
                if len(blob) < 1_000:
                    raise ValueError("heard nothing")
                import tempfile
                import transcribe as TR
                whisper, ff = TR._bin("mlx_whisper"), TR._bin("ffmpeg")
                if not whisper or not ff:
                    raise ValueError("no transcriber on this Mac — "
                                     "mlx_whisper and ffmpeg are needed")
                with tempfile.TemporaryDirectory() as td:
                    ...
                    r = subprocess.run([whisper, wav, "--model", TR.MODEL,
                                        "--task", "transcribe",
                                        "--output-format", "txt",
                                        "--output-dir", td,
                                        "--output-name", "out"],
                    ...
                    if r.returncode != 0 or not os.path.exists(outp):
                        raise ValueError("transcription failed on this Mac")
