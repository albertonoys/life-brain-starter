...
def sync():
    ...
    for src in sources:
        ...
        age = "unknown" if days is None else (
            "today" if days == 0 else f"{days} day{'s' if days != 1 else ''} ago")
        rows.append(f"| {name} | {age} | {len(items)} | {last or '—'} |")
        ...
        if items:
            ...
            for it in shown:
                blocks.append(f"- [ ] {it['text']}")
            if len(items) > len(shown):
                blocks.append(f"- *...and {len(items) - len(shown)} more in that folder.*")
        else:
            where = ", ".join(f"`{f}`" for f in files) if files else "no checklist files"
            blocks.append(f"No open checkboxes found ({where}).")
        blocks.append("")
    ...
    with open(OUT, "w", encoding="utf-8") as f:
        f.write("\n".join(out).rstrip() + "\n")
    return len(sources) - len(missing), total_items, brains
