...
if ($MorningRun -eq "skip" -or -not $Claude) {
    ...
    # older than six hours. (On Mac/Linux the scripts also chmod the
    # private paths unreadable; Windows has no chmod that denies reads,
    # so the hook is the only layer here.)
...
python3 brain/tools/telegram_bridge.py --push-plan 2>&1 | Add-Content $Log
python3 brain/tools/telegram_bridge.py --push-news 2>&1 | Add-Content $Log
