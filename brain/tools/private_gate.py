...
On Windows chmod cannot deny reads, so only the hook layer exists there;
the docstring of lock() says so where it matters.
...
def lock():
    ...
    run. chmod 0 on the top entry is enough — an unreadable directory
    refuses everything inside it. No-op on Windows, where chmod cannot
    deny reads (only the hook layer exists there)."""
    if os.name == "nt":
        return 0
    state = {}
    for ap in _targets():
...
def unlock():
    ...
    the state file gone it restores sane modes on anything still locked."""
    if os.name == "nt":
        return 0
    try:
